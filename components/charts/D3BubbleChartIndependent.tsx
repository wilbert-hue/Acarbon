...
export function D3BubbleChartIndependent({ title, height = 500 }: BubbleChartProps) {
  ...
  const unit = activeFilters.dataType === 'value'
    ? isINR
      ? currencySymbol
  ...
  return (
    <div className="w-full min-w-0 overflow-hidden" ref={containerRef}>
      ...
      <div className="mb-4 space-y-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
        ...
        {/* Value vs volume — applies to Demand Analysis and Coherent Opportunity Matrix */}
        <div>
          <label className="mb-2 block text-sm font-medium text-black">Data type</label>
          <div className="flex max-w-md rounded-lg border border-gray-200 bg-white p-0.5">
            <button
              type="button"
            ...
            <button
              ...
              title={
                data?.metadata?.has_volume
                  ? 'Plot bubbles using volume (tons)'
                  : 'No volume data for this dataset'
              }
              className={`flex-1 rounded-md px-3 py-2 text-sm font-medium transition-colors ${
          ...
          <p className="mt-1 text-xs text-gray-600">
            {activeFilters.dataType === 'value'
              ? `Market value (${selectedCurrency} ${unitText})`
